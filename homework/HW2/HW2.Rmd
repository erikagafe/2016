...
X <- sample( c(-1,1), N, replace = TRUE, prob=c(10/19, 9/19))
p_hat <- mean(X==1)
cat("N is", N, "Our estimate of the percent of black is", p_hat, "plus or minus", round( sqrt(p_hat*(1-p_hat)/N), 4))
...
The expected value changes depeding the N. However the Standard deviation becomes smaller everytime that N increases.
...
N<-25
B <- 10^4
error <- replicate(B,{X <- sample(c(-1,1), N, replace = TRUE, prob=c(10/19, 9/19))
  p_hat<-mean(X==1)
})
p_hat
hist(error)
...
```{r}
bank<-sample( c(1,-1), 1000, replace = TRUE, prob=c(.80, .20))
sum(bank==-1)
